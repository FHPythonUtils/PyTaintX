...
from pytaintx.usage import (default_blackbox_mapping_file,
default_trigger_word_file)
from pytaintx.vulnerabilities import find_vulnerabilities, vulnerabilities
from pytaintx.vulnerabilities.trigger_definitions_parser import (Sink,
Source,
parse)
from pytaintx.web_frameworks import (FrameworkAdaptor,
...
class EngineTest(VulnerabilitiesBaseTestCase):
	def test_parse(self):
		definitions = parse(trigger_word_file=os.path.join(
		os.getcwd(), 'pytaintx', 'vulnerability_definitions', 'test_triggers.pyt'))
	...
	def test_find_sanitiser_nodes(self):
		...
		result = list(
		vulnerabilities.find_sanitiser_nodes(sanitiser, [sanitiser_tuple]))
	...
	def test_XSS_result(self):
		...
		self.assertAlphaEqual(vulnerability_description,
		EXPECTED_VULNERABILITY_DESCRIPTION)

	def test_command_injection_result(self):
		vulnerabilities = self.run_analysis(
		'examples/vulnerable_code/command_injection.py')
		self.assert_length(vulnerabilities, expected_length=1)
		vulnerability_description = str(vulnerabilities[0])
		EXPECTED_VULNERABILITY_DESCRIPTION = """
	...
	def test_path_traversal_result(self):
		vulnerabilities = self.run_analysis(
		'examples/vulnerable_code/path_traversal.py')
		self.assert_length(vulnerabilities, expected_length=1)
		vulnerability_description = str(vulnerabilities[0])
		EXPECTED_VULNERABILITY_DESCRIPTION = """
	...
	def test_ensure_saved_scope(self):
		vulnerabilities = self.run_analysis(
		'examples/vulnerable_code/ensure_saved_scope.py')
		self.assert_length(vulnerabilities, expected_length=1)
		vulnerability_description = str(vulnerabilities[0])
		EXPECTED_VULNERABILITY_DESCRIPTION = """
	...
	def test_path_traversal_sanitised_result(self):
		vulnerabilities = self.run_analysis(
		'examples/vulnerable_code/path_traversal_sanitised.py')
		self.assert_length(vulnerabilities, expected_length=1)
		vulnerability_description = str(vulnerabilities[0])
		EXPECTED_VULNERABILITY_DESCRIPTION = """
	...
	def test_path_traversal_sanitised_2_result(self):
		vulnerabilities = self.run_analysis(
		'examples/vulnerable_code/path_traversal_sanitised_2.py')
		self.assert_length(vulnerabilities, expected_length=1)
		vulnerability_description = str(vulnerabilities[0])
		EXPECTED_VULNERABILITY_DESCRIPTION = """
	...
	def test_XSS_variable_assign_no_vuln_result(self):
		vulnerabilities = self.run_analysis(
		'examples/vulnerable_code/XSS_variable_assign_no_vuln.py')
		self.assert_length(vulnerabilities, expected_length=0)

	def test_XSS_variable_assign_result(self):
		vulnerabilities = self.run_analysis(
		'examples/vulnerable_code/XSS_variable_assign.py')
		self.assert_length(vulnerabilities, expected_length=1)
		vulnerability_description = str(vulnerabilities[0])
		EXPECTED_VULNERABILITY_DESCRIPTION = """
	...
	def test_XSS_variable_multiple_assign_result(self):
		vulnerabilities = self.run_analysis(
		'examples/vulnerable_code/XSS_variable_multiple_assign.py')
		self.assert_length(vulnerabilities, expected_length=1)
		vulnerability_description = str(vulnerabilities[0])
		EXPECTED_VULNERABILITY_DESCRIPTION = """
	...
	def test_list_append_taints_list(self):
		vulnerabilities = self.run_analysis('examples/vulnerable_code/list_append.py',
...
class EngineEveryTest(VulnerabilitiesBaseTestCase):
	...
	def test_self_is_not_tainted(self):
		vulnerabilities = self.run_analysis(
		'examples/example_inputs/def_with_self_as_first_arg.py')
		self.assert_length(vulnerabilities, expected_length=0)
...
class EnginePositionTest(VulnerabilitiesBaseTestCase):
	...
	def test_args_kwargs_that_do_dont_propagate(self):
		def check(fixture, vulnerable):
			...
			self.cfg_create_from_ast(tree)
			vulnerabilities = self.run_analysis()
			self.assert_length(vulnerabilities,
			expected_length=1 if vulnerable else 0,
			msg=fixture)

		no_vuln_fixtures = ('execute(0)',
		'run(0, x, TAINT, 0, x=x)',
