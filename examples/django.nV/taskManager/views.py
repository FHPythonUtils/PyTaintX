...
def download(request, file_id):
	...
	abspath = open(
	os.path.dirname(os.path.realpath(__file__)) + file.path,
	'rb',
	encoding="utf-8")
...
def task_edit(request, project_id, task_id):
	...
	if request.method == 'POST':
		...
		return render_to_response('taskManager/task_edit.html', {'task': task},
		RequestContext(request))
...
def project_create(request):
	...
	if request.method == 'POST':
		...
		now = timezone.now()
		project_duedate = timezone.make_aware(
		datetime.datetime
		.fromtimestamp(int(request.POST.get('project_duedate', False))))
		...
		return render_to_response('taskManager/project_create.html', {},
		RequestContext(request))
...
def project_edit(request, project_id):
	...
	if request.method == 'POST':
		...
		return render_to_response('taskManager/project_edit.html', {'proj': proj},
		RequestContext(request))
...
def login(request):
	if request.method == 'POST':
		...
		return render_to_response('taskManager/login.html', {},
		RequestContext(request))
...
def register(request):
	...
	return render_to_response('taskManager/register.html',
...
def project_details(request, project_id):
	...
	if not proj:
		...
		return render(request,
		'taskManager/project_details.html',
...
def note_create(request, project_id, task_id):
	if request.method == 'POST':
		...
		note = Notes(title=note_title, text=text, user=request.user, task=parent_task)

		note.save()
		return redirect('/taskManager/' + project_id + '/' + task_id,
		{'new_note_added': True})
	else:
		return render_to_response('taskManager/note_create.html',
		{'task_id': task_id},
		RequestContext(request))
...
def note_edit(request, project_id, task_id, note_id):
	...
	if request.method == 'POST':
		...
		return render_to_response('taskManager/note_edit.html', {'note': note},
		RequestContext(request))
...
def search(request):
	...
	my_task_list = Task.objects.filter(users_assigned=request.user.id).filter(
	title__icontains=query).order_by('title')
	return render(
	request,
...
def forgot_password(request):
	...
	if request.method == 'POST':
		...
		try:
			...
			"Reset your password",
			"You can reset your password at /taskManager/reset_password/. Use \"{}\" as your token. This link will only work for 10 minutes."
			.format(reset_token))
...
def change_password(request):
	...
	return render(request,
	'taskManager/change_password.html', {'user': request.user})


def tm_settings(request):
	settings_list = request.META
	return render(request,
	'taskManager/settings.html', {'settings': settings_list})
