...
from ..core.node_types import AssignmentNode, BBorBInode, IfNode, TaintedNode
from ..helper_visitors import CallVisitor, RHSVisitor, VarsVisitor
from .trigger_definitions_parser import Source, parse
from .vulnerability_helper import (Sanitiser,
TriggerNode,
Triggers,
VulnerabilityType,
vuln_factory)
...
def identify_triggers(cfg, sources, sinks, lattice, nosec_lines):
	...
	TriggerNode(Source('Framework function URL parameter'), cfg_node=node)
	...
	for i, current_node in enumerate(chain):
		...
		if isinstance(current_node, BBorBInode):
			if current_node.func_name in blackbox_mapping['propagates']:
				...
				user_says = input(
				'Is the return value of {} with tainted argument "{}" vulnerable? ([Y]es/[N]o/[S]top asking)'
				.format(current_node.label, chain[i - 1].left_hand_side)).lower()
...
source, sink, triggers, lattice, cfg, interactive, blackbox_mapping):
    ...
    """
	nodes_in_constraint = [
	secondary for secondary in reversed(source.secondary_nodes)
	...
	if tainted_node_in_sink_arg:
		...
		for chain in get_vulnerability_chains(source.cfg_node, sink.cfg_node,
		def_use):
...
def find_vulnerabilities_in_cfg(cfg,
definitions,
lattice,
blackbox_mapping,
...
nosec_lines=defaultdict(set)):
	...
	for cfg in cfg_list:
		find_vulnerabilities_in_cfg(cfg,
		definitions,
		Lattice(cfg.nodes),
		blackbox_mapping,
		vulnerabilities,
