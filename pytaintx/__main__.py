...
def retrieve_nosec_lines(path):
	...
	return set(lineno for (lineno, line) in enumerate(lines, start=1)
...
def main(command_line_args=sys.argv[1:]): # noqa: C901
	...
	logging_level = (logging.ERROR if not args.verbose else logging.WARN
	if args.verbose == 1 else logging.INFO if args.verbose == 2 else logging.DEBUG)
	...
	if args.baseline:
		vulnerabilities = get_vulnerabilities_not_in_baseline(vulnerabilities,
		args.baseline)

	args.formatter.report(vulnerabilities,
	args.output_file,
	not args.only_unsanitised)

	has_unsanitised_vulnerabilities = any(not isinstance(v, SanitisedVulnerability)
