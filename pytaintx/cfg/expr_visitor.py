...
from ..core.ast_helper import Arguments, get_call_names_as_string
...
ReturnNode,
YieldNode)
from ..helper_visitors import LabelVisitor, RHSVisitor
from .alias_helper import handle_aliases_in_calls
from .expr_visitor_helper import (BUILTINS,
MUTATORS,
SavedVariable,
...
class ExprVisitor(StmtVisitor):
	...
	def save_local_scope(self, line_number, saved_function_call_index):
		...
		if (type(node) == AssignmentNode or type(node) == AssignmentCallNode
		or type(Node) == BBorBInode)]: # type() is used on purpose here
	...
	self, call_args, def_args, line_number, saved_function_call_index, first_node):
		...
		for i, call_arg in enumerate(call_args):
			...
			def_arg_temp_name = 'temp_' + str(
			saved_function_call_index) + '_' + def_args[i]
	...
	def restore_saved_local_scope(self, saved_variables, args_mapping, line_number):
		...
		for var in saved_variables:
			...
			if var.RHS in args_mapping:
				...
				RestoreNode(var.RHS + ' = ' + args_mapping[var.RHS],
				var.RHS, [var.LHS],
				line_number=line_number,
				path=self.filenames[-1]))
			else:
	...
	def process_function(self, call_node, definition):
		...
		self.filenames.pop(
		) # Should really probably move after restore_saved_local_scope!!!
	...
	def visit_Call(self, node):
		...
		and isinstance(node.func, ast.Attribute)):
			...
			fake_aug_assign = ast.AugAssign(target=node.func.value,
			op=ast.Add,
			value=node,
																																		)
