...
from ..core.module_definitions import (LocalModuleDefinition,
ModuleDefinition,
ModuleDefinitions)
from ..core.node_types import (AssignmentCallNode,
AssignmentNode,
BBorBInode,
BreakNode,
ControlFlowNode,
...
from ..helper_visitors import LabelVisitor, RHSVisitor, VarsVisitor
from .alias_helper import (as_alias_handler,
...
uninspectable_modules = {module.name
for module in iter_modules()} # Don't warn about failing to import these
...
class StmtVisitor(ast.NodeVisitor):
	...
	def visit_If(self, node):
		...
		if isinstance(body_connect_stmts, IgnoredNode):
			body_connect_stmts = ConnectStatements(first_statement=test,
			last_statements=[],
			break_statements=[])
		...
		if node.orelse:
			...
			test) # if there is no orelse, test needs an edge to the next_node
	...
	def visit_Return(self, node):
		...
		if isinstance(node.value, ast.Call):
			...
			LHS,
			node, [return_value_of_call.left_hand_side],
	...
	def handle_stmt_star_ignore_node(self, body, fallback_cfg_node):
		try:
			...
			body = ConnectStatements(first_statement=[fallback_cfg_node],
	...
	def visit_Try(self, node):
		...
		return ControlFlowNode(try_node,
		last_statements,
		break_statements=body.break_statements)

	def assign_tuple_target(self,
	target_nodes,
	value_nodes,
	...
	def visit_Assign(self, node):
		...
		if isinstance(node.targets[0], (ast.Tuple, ast.List)): # x,y = [1,2]
			if isinstance(node.value, (ast.Tuple, ast.List)):
				...
				return self.assign_tuple_target(node.targets[0].elts, [value_node],
				rhs_visitor.result)
			else:
				label = LabelVisitor()
				label.visit(node)
	...
	def visit_For(self, node):
		...
		Node("for " + target_label.result + " in " + iterator_label.result + ':',
		node,
		path=self.filenames[-1]))
	...
	def process_loop_funcs(self, comp_n, loop_node):
        ...
        :return: None
        """
		if isinstance(comp_n, ast.Call) and get_call_names_as_string(
		comp_n.func) in self.function_names:
	...
	def add_blackbox_or_builtin_call(self, node, blackbox): # noqa: C901
		...
		call_function_label = call_label_visitor.result[:call_label_visitor.result
		.find('(')]
		...
		for arg_node in itertools.chain(node.args, node.keywords):
			...
			if isinstance(arg, ast.Call):
				...
				if isinstance(arg_node, ast.keyword) and arg_node.arg is not None:
					visual_args.append(arg_node.arg + '='
	...
	def visit_With(self, node):
		...
		return ControlFlowNode(with_node,
	...
	from_fdid=False):
		...
		new_module_definitions = ModuleDefinitions(local_names,
		module_or_package_name)
	...
	self, module, real_names, local_names, import_alias_mapping, skip_init=False):
		...
		if init_exists and not skip_init:
			...
			return self.add_module(module=(module[0], init_file_location),
		...
		for real_name in real_names:
			...
			if os.path.isdir(full_name):
				...
				if os.path.isfile(new_init_file_location):
					self.add_module(module=(real_name, new_init_file_location),
	...
	def visit_ImportFrom(self, node):
		...
		for name in node.names:
			local_definitions = self.module_definitions_stack[-1]
			local_definitions.import_alias_mapping[name.asname
			or name.name] = f"{node.module}.{name.name}"
